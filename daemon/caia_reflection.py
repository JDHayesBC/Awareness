#!/usr/bin/env python3
"""
Caia Reflection Daemon - Autonomous reflection and maintenance.
...
Adapted from lyra_reflection.py for Caia's entity context.
...
# Configuration
ENTITY_PATH = os.getenv("ENTITY_PATH", "/mnt/c/Users/Jeff/Claude_Projects/Awareness/entities/caia")
CONVERSATION_DB_PATH = os.getenv("CONVERSATION_DB_PATH", f"{ENTITY_PATH}/data/conversations.db")
JOURNAL_PATH = os.getenv("REFLECTION_JOURNAL_PATH", f"{ENTITY_PATH}/journals/reflection")
...
# Reflection runs from its own directory to isolate sessions
REFLECTION_CWD = Path(os.getenv("REFLECTION_CWD", str(Path(__file__).parent / "reflect-caia")))


class CaiaReflectionDaemon:
    """Daemon for autonomous reflection - runs independently of Haven bot."""
    ...
    def __init__(self):
        ...
        print(f"[INIT] Caia reflection daemon initialized")
        print(f"[INIT] Entity: {ENTITY_PATH}")
    ...
    async def start(self):
        ...
        print("[START] Caia reflection daemon starting...")
    ...
    async def stop(self):
        """Stop the daemon."""
        print("[STOP] Caia reflection daemon stopping...")
        self.running = False
...
Someone is currently working on the project in terminal.

**Project is locked**: {lock_info.get('context', 'Terminal session active')}

Since the project is locked, focus on other activities:
...
Avoid editing project files or making commits while the project is locked.
